...
export function hintLadder(level: HintLevel, flow: FlowState, result?: Evaluation): string[] {
  const socket = pendingSockets(level, flow)[0]
  if (!socket) return []
  const answer = socket.inventory.find((p) => socket.options[p]?.outcome === 'solves')
  if (!answer) return []
  const where = level.sockets.length > 1 ? ` (socket "${socket.label}")` : ''
  ...
  return [
    (failing && QUESTIONS[failing]) ?? 'Corre el circuito y sigue un pulso: ¿dónde se tuerce el recorrido?',
    `El patrón que buscas${where} es de la familia ${FAMILY_NAMES[family]}: ${FAMILY_HINT[family]}.`,
    ...(decoy ? [`Descarta ${PATTERNS[decoy].name}${where}: no encaja con este problema.`] : []),
    `Prueba con ${PATTERNS[answer].name}${where}.`,
  ]
}
